...
pheno$Status <- factor(pheno$Status, levels = c("AD", "CO", "Other"))
...
cont_table <- data.frame(matrix(nrow=2, ncol=3))
...
fisher_linear_res <- ldply(setdiff(fisher_results_sig$metab, "donepezil"), get_effect_pval, model_data_ad, "AD")
...
##### Recover retinol and 6-oxopiperidine-2-carboxylate because of cognitive differences
recover <- c(recover, "retinol (Vitamin A)", "6-oxopiperidine-2-carboxylate")
...
# Log transform readings, median has been scaled to 1
