...
# Data from ADNIMERGE
FIA_ADNI2 <- admcdukep180fiaadni2go
UPLC_ADNI2 <- admcdukep180uplcadni2go

# Blood was obtained at baseline visit, hence viscode == "bl"
pheno <- adnimerge %>%
  filter(RID %in% FIA_ADNI2$RID, VISCODE == "bl") %>%
  select(
...
demog_small <- ptdemog %>% select(RID, PTDOB) %>% unique() %>% na.omit()
...
FIA_ADNI2 <- filter(FIA_ADNI2, RID %in% pheno$RID)[,c(2, 27:167)] %>%
  mutate_at(vars(setdiff(colnames(.), "RID")), na_if, "< LOD") %>%
  mutate_if(is.character, as.numeric)
UPLC_ADNI2 <- filter(UPLC_ADNI2, RID %in% pheno$RID)[,c(2, 27:67)] %>%
  mutate_at(vars(setdiff(colnames(.), "RID")), na_if, "< LOD") %>%
  mutate_if(is.character, as.numeric)

# Average the replicates
replicates <- unique(FIA_ADNI2$RID[duplicated(FIA_ADNI2$RID)])

# Average replicates
replicates_only_FIA <- FIA_ADNI2[FIA_ADNI2$RID %in% replicates,]
replicates_only_UPLC <- UPLC_ADNI2[UPLC_ADNI2$RID %in% replicates,]
...
for (rep_id in replicates) {
  for (i in 2:ncol(replicates_only_UPLC)) {
    ...
    if (sum(!is.na(pair)) > 1) {
      ...
      distrib <- quantile(UPLC_ADNI2[,i], c(0.1), na.rm = TRUE)
...
uplc_noreps <- 
  UPLC_ADNI2[!UPLC_ADNI2$RID %in% replicates,]
UPLC_ADNI2_avgreps <- rbind(uplc_noreps, uplc_avgs)
...
for (rep_id in replicates) {
  for (i in 2:ncol(replicates_only_FIA)) {
    ...
    if (sum(!is.na(pair)) > 1) {
      ...
      distrib <- quantile(FIA_ADNI2[,i], c(0.1), na.rm = TRUE)
      if (pair[!is.na(pair)] < distrib) avgs[i] <- pair[!is.na(pair)]
      else avgs[i] <- NA
    }
    else avgs[i] <- NA
...
fia_noreps <- 
  FIA_ADNI2[!UPLC_ADNI2$RID %in% replicates,]
FIA_ADNI2_avgreps <- rbind(fia_noreps, fia_avgs)

# Create data frames for models
model_data_all <- # 172 metabs, log transformed and mean adjusted
  pheno %>%
  right_join(FIA_ADNI2_avgreps) %>%
  right_join(UPLC_ADNI2_avgreps)
...
nrow(missing20_all) # 24 analytes
...
# ADNI2GO
ADNI2GO_only_metabs <- model_data_ad[,-1:-5]

test2 <- c()
for (i in 1:nrow(ADNI2GO_only_metabs)) {
  test2[i] <- sum(is.na(ADNI2GO_only_metabs[i,]))/ncol(ADNI2GO_only_metabs)
}

# Remove subject missing breater than 20% of readings
model_data_ad$RID[which(test2 > .2)]
model_data_ad <- model_data_ad[-which(test2 > .2),]

# saveRDS(model_data_ad, "data/11-model_data_ad_ADNI2GO.rds")


# Create data frame with effects, pvals, and padj for each metabolite
ad_effect_pval_df <- plyr::ldply(setdiff(colnames(model_data_ad[,-1:-5]), "PC.ae.C30.1"), 
                                 get_effect_pval,
                                 model_data = model_data_ad, 
                                 status = "AD")
ad_effect_pval_df$padj <- p.adjust(ad_effect_pval_df$pval, method = "BH")
# saveRDS(ad_effect_pval_df, "data/11-effect_pval_ADNI2GO.rds")
